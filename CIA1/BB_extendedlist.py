...
def branch_and_bound_EL(d, start, end, oracle):
    ...
    while queue:
        ...
        for neighbor, cost in d[current_node].items():
            if neighbor in current_path: 
                continue
            
            if current_path[0] + cost <= oracle and neighbor not in vis:
                vis.add(neighbor)
...
def oracle_search(d, start, end, factor):
    return sum([min(cost.values()) for cost in d.values()]) * factor
...
oracle_value = oracle_search(d, 'S', 'G', 2)
final_route = branch_and_bound_EL(d, 'S', 'G', oracle_value)

g = form_graph(d, final_route[0][1:])  
Viz(g)
